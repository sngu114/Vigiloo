...
export default function SocialMediaHub() {
  const platforms = [
    {
      name: "Instagram",
      imageUrl: "/instagram-preview.png", // Replace with your image path
    ...
    {
      name: "Snapchat",
      imageUrl: "/snapchat-preview.png", // Replace with your image path
    ...
    {
      name: "TikTok",
      imageUrl: "/tiktok-preview.png", // Replace with your image path
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-16">
          {platforms.map((platform) => (
            <div key={platform.name} className="bg-white p-10 rounded-[40px] border border-gray-100 shadow-sm text-center flex flex-col items-center group hover:shadow-2xl hover:border-[#F0EBFF] transition-all duration-300">
              ...
              {/* Image Box Replacement for Emoji */}
              <div 
                className={`w-24 h-24 ${platform.color} rounded-3xl mb-8 overflow-hidden flex items-center justify-center border border-gray-100 group-hover:scale-110 transition-transform duration-300 shadow-inner`}
                style={{
              ...
              >
                ...
                {!platform.imageUrl && <span className="text-gray-300 text-xs">No Image</span>}
              </div>

              <h3 className="text-2xl font-bold mb-4 text-[#0F172A]">{platform.name}</h3>
              <p className="text-gray-500 leading-relaxed mb-10 flex-grow px-2">{platform.desc}</p>
